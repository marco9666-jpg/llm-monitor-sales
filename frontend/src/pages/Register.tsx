...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || t('register.error'))
    ...
    <div className="flex min-h-screen items-center justify-center bg-neutral-950 px-6">
      <div className="w-full max-w-sm">
        <div className="mb-10 text-center">
          ...
          <h1 className="text-2xl font-semibold text-white tracking-tight">{t('register.title')}</h1>
          <p className="mt-2 text-sm font-light text-neutral-500">{t('register.subtitle')}</p>
        </div>
        ...
        <div className="rounded-3xl border border-white/10 bg-neutral-900/50 p-8 backdrop-blur-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label className="mb-2 block text-sm font-medium text-neutral-400">{t('register.name')}</label>
              <input
                type="text"
                required
                value={name}
            ...
            <div>
              <label className="mb-2 block text-sm font-medium text-neutral-400">{t('register.email')}</label>
              <input
                type="email"
                required
                value={email}
            ...
            <div>
              <label className="mb-2 block text-sm font-medium text-neutral-400">{t('register.password')}</label>
